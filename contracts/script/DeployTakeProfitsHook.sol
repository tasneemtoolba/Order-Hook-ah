...
contract DeployTakeProfitsHook is Script, Deployers {
    ...
    function run() public {
        ...
        // Only proceed if hook deployment was successful
            ...
            token0.approve(address(UNIVERSAL_ROUTER), type(uint256).max);
            token1.approve(address(UNIVERSAL_ROUTER), type(uint256).max);
            token0.approve(address(POSITION_MANAGER), type(uint256).max);
            token1.approve(address(POSITION_MANAGER), type(uint256).max);
       

        // Only proceed with pool initialization and liquidity addition if hook deployment was successful
        // if (hookAddress != address(0)) {
     

// Log all the key components
console.log("Pool Key Components:");

        // Set up the manager and routers with the correct addresses
        manager = IPoolManager(POOL_MANAGER);
        modifyLiquidityRouter = PoolModifyLiquidityTest(POOL_MODIFY_LIQUIDITY_TEST);
        ...
        IPoolManager(POOL_MANAGER).initialize(pool, 4411237397794263893240602165248);
